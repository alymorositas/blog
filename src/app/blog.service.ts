import { Post, Category, Favorite } from './blog';
...
export class BlogService {
  ...
  getPosts(): Post[] {
    return [
      ...
      },
      {
        id: 9,
        title:
          'Quia dolores debitis voluptas sunt voluptatum aperiam perferendis.',
  ...
  favorites: Favorite[] = [];
  aggiungi(post: Favorite) {
    let p = this.favorites.find((x) => x.id == post.id);
  ...
  cancelFav(f: Favorite) {
    _.remove(this.favorites, f);
  }
  ...
  arrayPosts = new Array();

  createArray(id: string) {
    let arrayFiltered = this.getPosts().filter((obj) => {
      return obj.category === id;
    });
    this.arrayPosts = arrayFiltered;
